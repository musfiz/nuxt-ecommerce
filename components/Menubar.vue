      ...
      <div class="menu-item">
        <div class="menu-item-group" v-for="menu in menus" :key="menu.id">
          <div class="item-parent" @click="toggleChildMenu(menu)">
            ...
            <NuxtLink v-if="$i18n.locale == 'en'" :to="menu.route" :class="{ 'nuxt-link-active': menu.activeClass }"
              @click="storeSelectedMenuInStore(menu)">{{ menu.title }}</NuxtLink>
            <NuxtLink v-else :to="menu.route" :class="{ 'nuxt-link-active': menu.activeClass }"
              @click="storeSelectedMenuInStore(menu)">{{ menu.titleBn }}</NuxtLink>
            <img v-if="menu.child.list" class="child-menu-icon" src="~/assets/frontend/chevron-right.svg" alt="No Image"
              :class="{ 'rotate': menu.child.rotate }">
          </div>
          <div v-if="menu.child.list" class="item-child-group" :class="{ 'active': menu.activeClass }">
            <div class="item-child" v-for="child in menu.child.list" :key="child.id">
              <NuxtLink v-if="$i18n.locale == 'en'" :to="child.route" :class="{ 'nuxt-link-active': child.activeClass }"
                @click="toggleChildMenu(menu, child)">{{ child.title }}</NuxtLink>
              <NuxtLink v-else :to="child.route" :class="{ 'nuxt-link-active': child.activeClass }"
                @click="toggleChildMenu(menu, child)">{{ child.titleBn }}</NuxtLink>
            </div>
          </div>
        </div>
      </div>
